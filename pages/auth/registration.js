...
export default function AddAdmin() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            setSuccess('You are  add successfully');
            reset();
            ...
            setSuccess(' Sorry Can u try again ' + error.response.data.message);
        }
    };
...
<input type="file" id="myfile" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer
...
{errors.myfile && 
  ...
  <p id="outlined_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400"><span class="font-medium">invalid file</span></p>
                          ...
                          font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Submit</button>
              </form>
          </div>
      </div>
  </div>
